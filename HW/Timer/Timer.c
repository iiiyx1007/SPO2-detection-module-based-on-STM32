...
* 注    意：APB1时钟为36MHz，TIM2-TIM5时钟选择为APB1的2倍，因此，TIM2-TIM5时钟为72MHz
*********************************************************************************************************/
static  void ConfigTimer6(u16 arr,u16 psc)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;//TIM_TimeBaseStructure用于存放定时器的参数
  NVIC_InitTypeDef NVIC_InitStructure;           //NVIC_InitStructure用于存放NVIC的参数
  ...
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;       //设置时钟分割：tDTS = tCK_INT
  TIM_TimeBaseStructure.TIM_CounterMode   = TIM_CounterMode_Up; //设置向上计数模式
  TIM_TimeBaseInit(TIM6, &TIM_TimeBaseStructure);               //根据参数初始化定时器
 
  TIM_ITConfig(TIM6,TIM_IT_Update,ENABLE);                      //使能定时器的更新中断

  //配置NVIC
  NVIC_InitStructure.NVIC_IRQChannel = TIM6_IRQn;               //中断通道号
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;     //设置抢占优先级
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 3;            //设置子优先级
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;               //使能中断
  NVIC_Init(&NVIC_InitStructure);                               //根据参数初始化NVIC 
...
{
  static  i16 s_iCnt100  = 0;                        //定义一个静态变量s_iCnt1000作为1s计数器
  ...
  s_iCnt100++;           //1000ms计数器的计数值加1
                                                      
  if(s_iCnt100 >= 30)  //1000ms计数器的计数值大于或等于1000
  {                                                   
    s_iCnt100 = 0;       //重置1000ms计数器的计数值为0
...
{  
	static  u16 s_iCnt2 = 0;                            //定义一个静态变量s_iCnt2作为2ms计数器
  if(TIM_GetITStatus(TIM6, TIM_IT_Update) == SET)     //判断定时器更新中断是否发生
  {
    TIM_ClearITPendingBit(TIM6, TIM_FLAG_Update);     //清除定时器更新中断标志
  }
	s_iCnt2++;            //2ms计数器的计数值加1
	if(s_iCnt2 >= 2)      //2ms计数器的计数值大于或等于2
  {                                                   
    s_iCnt2 = 0;        //重置2ms计数器的计数值为0
		s_i500usFlag = TRUE;  //将0.5ms标志位的值设置为TRUE 
  }
}
...
void InitTimer(void)
{
  ConfigTimer2(999, 71);  //72MHz/(71+1)=1MHz，由0计数到499为0.5ms
  ConfigTimer5(999, 71);  //72MHz/(71+1)=1MHz，由0计数到999为1ms
	ConfigTimer6(499, 71);  //72MHz/(71+1)=1MHz，由0计数到999为1ms
}
